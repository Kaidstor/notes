...
которой заметка устарела (сравнивается с датой по часам сервера): она пропадает из
индекса и счётчиков тегов, `/api/notes` отдаёт её только со `stale=1` и считает в
`staleCount`, а страница открывается как обычно, с плашкой «Устарела с …».
