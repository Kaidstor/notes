...
любой из токенов (чип «поделиться» на странице, `--share` в CLI) на срок от минуты до
...
Работать с задачами удобнее через CLI `notes-kai` (`cli/notes-kai.ts`), собирается в
один бинарник:

```bash
bun run install:cli              # → ~/.local/bin/notes-kai
notes-kai due --human            # что пора делать сегодня
notes-kai add task.md --due +3d  # завести: due и тег дописываются во frontmatter
...
`--human` — для глаз. Токен — `$NOTES_ADMIN_TOKEN`, иначе `sec get notes/NOTES_ADMIN_TOKEN`.
