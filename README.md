...
заметную фичу, дописывай запись туда же — порядок релиза описан в скилле
`skills/deploy-notes` (changelog → сборка → `./deploy.sh` → проверка → коммит).

Скиллы репозитория подключены в агента симлинками: `~/.claude/skills/publish-note` и
`~/.claude/skills/deploy-notes` указывают на `skills/*` здесь, копий нет.
