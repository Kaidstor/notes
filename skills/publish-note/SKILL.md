...
2. **Проверь схемы, если в заметке есть mermaid:**
   ...
   печатая `ok` либо первую строку ошибки. Запускать из репозитория `notes` — берёт оттуда
   же `mermaid` (та же версия, что на странице) и `@happy-dom/global-registrator`.

   **Ложная тревога, которую не надо чинить:** `Could not find a suitable point for the
   given distance` на схеме с ромбовидным узлом (`F{"Решение"}`) и подписанным ребром —
   это ограничение happy-dom, а не поломка схемы. В браузере такие схемы рисуются;
   воспроизводится на трёх строках (`A --> F{"Решение"}` + `F -->|"да"| B`). Увидел
   именно эту ошибку и именно на такой схеме — посмотри страницу глазами через `--local`
   и публикуй. Любой другой текст ошибки — настоящий, схему править.
3. **Покажи локально, если правки в вёрстке или заметка важная:**
   ```bash
...
- **Длинный линейный конвейер — в две колонки, а не в одну строку.** У цепочки из
  ...
  Проверять глазами, а не на глаз: соотношение сторон видно в `viewBox` готовой
  схемы на странице, целиться в 1.6 и ниже.
