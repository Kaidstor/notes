...
2. **Проверь схемы, если в заметке есть mermaid:**
   ...
   Ненулевой код возврата бывает только на настоящих ошибках. Известное ограничение
   happy-dom (`Could not find a suitable point for the given distance` на схеме с
   ромбовидным узлом и подписанным ребром) скрипт распознаёт сам и понижает до
   предупреждения: в браузере такие схемы рисуются. Увидел предупреждение — посмотри
   страницу через `--local` и публикуй.
   ...
   **Скрипт проверяет только синтаксис — соотношение сторон он поймать не может**, и
   ...
   Как мерить — пункт 3.
3. **Покажи локально, если правки в вёрстке, есть схемы или заметка важная:**
   ```bash
   ...
   **Есть схемы — здесь же померь соотношение сторон**, это единственное место, где оно
   доступно. Открыть локальный URL браузером (`mcp__chrome-devtools__new_page`) и выполнить
   на странице:
   ...
   их не трогать. Схема шире — свернуть этапы в подграфы с `direction TB` и перемерить.
   На реальной заметке линейная цепочка из семи узлов дала 7.86 и была нечитаема, те же
   узлы в двух подграфах — 1.36.
4. **Опубликуй:**
   ```bash
...
- **Длинный линейный конвейер — в две колонки, а не в одну строку.** У цепочки из
  ...
  видит и печатает `ok` в том числе на схеме с соотношением 7.86. Мерить на живой странице
  по рецепту из пункта 3 «Шагов», целиться в 1.6 и ниже.
