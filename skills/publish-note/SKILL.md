---
name: publish-note
description: Публикация markdown-заметки на notes.kaidstor.ru — страница получает адрес {домен}/{uuid} и вёрстку в стиле приложения sql-kai (zinc-палитра, sky-акцент, врезки .note/.warn/.ok, авто-тема). Использовать когда пользователь просит «опубликуй заметку», «выложи документ/доку», «залей на notes», «сделай страницу со ссылкой», «обнови опубликованную заметку», «покажи что опубликовано», «удали заметку», а также когда по итогам работы получился .md-документ и нужна шарибельная ссылка на него. Также описывает, как посмотреть страницу локально до публикации.
---
...
- `--list [запрос]` печатает по строке на заметку: `<uuid>  <дата>  <заголовок>` —
  режется `awk`/`cut` без парсинга JSON.
...
GFM: таблицы, списки, цитаты, код с языковым бейджем. Плюс **сырые HTML-блоки** —
