...
2. **Проверь схемы, если в заметке есть mermaid:**
   ...
   **Скрипт проверяет только синтаксис — соотношение сторон он поймать не может**, и
   пытаться научить его не стоит: под happy-dom `render` отдаёт пустую строку вместо SVG
   (в DOM его тоже не остаётся, контейнером не лечится), а без `viewBox` мерить нечего.
   Как мерить — пункт 4.
3. **Опубликуй:**
   ```bash
   ...
   с `--pin` обновляет её же. Итерации правок гоняются прямо по проду, локальный
   сервер для проверки поднимать не нужно (когда всё же нужен — см. «Запасной
   вариант: локальный сервер»).
4. **Есть схемы — померь соотношение сторон на опубликованной странице.**
   Открыть URL заметки браузером (`mcp__chrome-devtools__new_page`): у пользователя
   после разового входа кука живёт год, страница откроется без формы. Увидел форму
   входа — попроси пользователя войти один раз; **токен в форму сам не вставляй**,
   он осядет в аргументах тула и транскрипте. Дождаться отрисовки и выполнить:
   ...
   () => Array.from(document.querySelectorAll('svg'))
     ...
     .map((s, i) => {
       ...
       return { i, ratio: +(vb[2] / vb[3]).toFixed(2) };
     })
   ```

   Цель — 1.6 и ниже; у `gantt`, `timeline`, `gitGraph` и `journey` широкий кадр родной,
   их не трогать. Схема шире — свернуть этапы в подграфы с `direction TB` (или
   `flowchart TB` с `direction LR` в подграфах), переопубликовать тем же файлом и
   перемерить. На реальной заметке линейная цепочка из семи узлов дала 7.86 и была
   нечитаема, те же узлы в двух подграфах — 1.36.
5. **Отдай ссылку пользователю.** Публикация — внешнее действие: если пользователь
   явно не просил публиковать, сначала спроси (черновик для итераций — тоже публикация).

### Запасной вариант: локальный сервер

Нужен редко: правки вёрстки самого сервиса notes, работа без сети или заметка,
которую нельзя класть на прод даже черновиком.

```bash
cd ~/Projects/own/notes && PORT=3100 bun run start
sec run notes -- bun ~/.claude/skills/publish-note/scripts/publish.ts --host http://localhost:3100 путь/к/note.md
```

**Гоча: порт 3000 по умолчанию часто занят чужим dev-сервером** (например,
recon-front). Сервер notes при этом падает на `Bun.serve`, а `--local` молча бьёт
в чужой сервер и получает его 404 — выглядит как сломанная публикация. Поэтому
поднимать на явном порту (`PORT=3100`) и публиковать через `--host`, а не `--local`.
Локальный сервер без токенов в окружении открыт всем — мерить схемы на нём можно
тем же сниппетом из пункта 4.
...
| Флаг | Смысл |
| --- | --- |
| `--local` | публиковать на `http://localhost:3000` вместо прода (порт занят чужим сервером — используй `--host`, см. «Запасной вариант») |
...
- **Длинный линейный конвейер — в две колонки, а не в одну строку.** У цепочки из
  ...
  Проверять числом, а не на глаз, и **не полагаться на `check-mermaid.ts`** — он этого не
  видит и печатает `ok` в том числе на схеме с соотношением 7.86. Мерить на опубликованной
  странице по рецепту из пункта 4 «Шагов», целиться в 1.6 и ниже.
